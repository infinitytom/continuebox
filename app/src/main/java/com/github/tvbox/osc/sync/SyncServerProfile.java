...
public class SyncServerProfile {
    ...
    public static void save(SyncServerProfile p) { List<SyncServerProfile> x=all(); for(int i=x.size()-1;i>=0;i--) if(x.get(i).endpoint.equalsIgnoreCase(p.endpoint)) x.remove(i); x.add(0,p); while(x.size()>10)x.remove(x.size()-1); Hawk.put(PROFILES,new Gson().toJson(x)); }
    public static void activate(SyncServerProfile p) { p.lastUsed=System.currentTimeMillis(); Hawk.put("sync_endpoint",p.endpoint); Hawk.put("sync_token",p.token); save(p); }
}
