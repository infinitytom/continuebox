...
import com.github.tvbox.osc.sync.SyncLoginDialog;
import com.github.tvbox.osc.sync.TvboxSyncClient;
import com.github.tvbox.osc.sync.SyncServerProfile;
...
public class UserFragment extends BaseLazyFragment implements View.OnClickListener {
    ...
    private View tvSyncLogin;
    private View tvSyncSwitch;
    ...
    protected void onFragmentResume() {
        ...
        new android.os.Handler(android.os.Looper.getMainLooper()).postDelayed(() -> TvboxSyncClient.pull(), 800);
    ...
    protected void init() {
        ...
        tvSyncLogin = findViewById(R.id.tvSyncLogin);
        tvSyncSwitch = findViewById(R.id.tvSyncSwitch);
        ...
        tvSyncLogin.setOnClickListener(v -> SyncLoginDialog.show(mContext));
        tvSyncSwitch.setOnClickListener(v -> showSyncProfiles());
    ...
    private void showSyncProfiles() {
        List<SyncServerProfile> profiles = SyncServerProfile.all();
        if (profiles.isEmpty()) { SyncLoginDialog.show(mContext); return; }
        String[] names = new String[profiles.size()];
        for (int i = 0; i < profiles.size(); i++) names[i] = profiles.get(i).name + "  " + profiles.get(i).endpoint;
        new AlertDialog.Builder(mContext).setTitle("切换服务器/账号").setItems(names, (d, which) -> {
            SyncServerProfile.activate(profiles.get(which));
            Toast.makeText(mContext, "已切换：" + profiles.get(which).name, Toast.LENGTH_SHORT).show();
            TvboxSyncClient.pull();
        }).setNegativeButton("取消", null).setPositiveButton("新增登录", (d, w) -> SyncLoginDialog.show(mContext)).show();
    }
